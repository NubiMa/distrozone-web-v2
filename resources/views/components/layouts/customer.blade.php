        ...
        <div class="container mx-auto px-4 py-4">
            <div class="flex items-center justify-between gap-8">
                ...
                <a href="{{ route('customer.home') }}"
                    class="text-white font-display text-2xl font-bold tracking-tight">
                    DISTROZONE
                </a>

                <!-- Search Bar (Zalora-style capsule) -->
                <div class="flex-1 max-w-2xl">
                    <form action="{{ route('customer.catalog') }}" method="GET" class="relative">
                ...
                <!-- Action Icons (Customer) -->
                <div class="flex items-center gap-6">
                    <!-- Wishlist -->
                    <a href="{{ route('customer.wishlist') }}"
                        class="text-border hover:text-accent transition-fast relative">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            class="flex items-center gap-2 text-white hover:text-accent transition-fast">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-card py-2 border border-border">
                            <a href="{{ route('customer.orders') }}"
                                class="block px-4 py-2 text-sm text-gray-800 hover:bg-bg-secondary hover:text-accent transition-fast">
                                📦 Pesanan Saya
                            </a>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-fast">
                                    🚪 Keluar
                                </button>
                            </form>
                        </div>
        ...
        <nav class="border-t border-gray-800">
            <div class="container mx-auto px-4">
                <div class="flex items-center justify-center gap-8 py-4">
                    ...
                    <a href="{{ route('customer.catalog', ['category' => 'basic-tees']) }}"
                        class="text-border hover:text-accent uppercase text-sm font-semibold tracking-wider transition-fast">
                        Basic Tees
                    </a>
                    <a href="{{ route('customer.catalog', ['category' => 'graphic-tees']) }}"
                        class="text-border hover:text-accent uppercase text-sm font-semibold tracking-wider transition-fast">
                        Graphic Tees
                    </a>
                    <a href="{{ route('customer.catalog', ['category' => 'oversized']) }}"
                        class="text-border hover:text-accent uppercase text-sm font-semibold tracking-wider transition-fast">
                        Oversized
                    </a>
                    <a href="{{ route('customer.catalog', ['category' => 'premium']) }}"
                        class="text-border hover:text-accent uppercase text-sm font-semibold tracking-wider transition-fast">
                        Premium
                    </a>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        <li><a href="{{ route('customer.catalog') }}" class="hover:text-accent transition-fast">Semua
                                Produk</a></li>
                        <li><a href="{{ route('customer.catalog', ['category' => 'basic-tees']) }}"
                                class="hover:text-accent transition-fast">Basic Tees</a></li>
                        <li><a href="{{ route('customer.catalog', ['category' => 'graphic-tees']) }}"
                                class="hover:text-accent transition-fast">Graphic Tees</a></li>
                        <li><a href="{{ route('customer.catalog', ['category' => 'oversized']) }}"
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        <li><a href="{{ route('customer.orders') }}"
                                class="hover:text-accent transition-fast">Pesanan Saya</a></li>
                        <li><a href="{{ route('customer.wishlist') }}"
                                class="hover:text-accent transition-fast">Wishlist</a></li>
                        <li><a href="{{ route('customer.profile') }}"
                                class="hover:text-accent transition-fast">Profil</a></li>
                        <li><a href="{{ route('customer.addresses') }}"
                                class="hover:text-accent transition-fast">Alamat</a></li>
                    </ul>
                </div>
